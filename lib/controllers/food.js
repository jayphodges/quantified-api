...
const patchFood = (request, response) => {
  ...
  if(!name && !calories) {
    return response.status(422).send({ error: `You're missing a property` })
  }

  if(name) {
    Food.updateFoodName(id, name)
    .then(function(data){
      response.status(200).json({"id": id, "name": name, "calories": calories})
    })
  }

  if (calories) {
    Food.updateFoodCalories(id, calories)
    .then(function(data){
      response.status(200).json({"id": id, "name": name, "calories": calories})
    })
  }
