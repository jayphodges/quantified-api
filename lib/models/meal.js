...
const index = () => {
  return database.raw(`
      SELECT meals.*, json_agg(foods.*) AS foods
      FROM meals
          JOIN foods_meals
...
const destroy = (food_id, meal_id) => {
  ...
  return database.raw('DELETE FROM foods_meals WHERE food_id = ? AND meal_id = ?', [food_id, meal_id])
  // return database.raw(`DELETE FROM foods_meals WHERE food_id = ${food_id} AND meal_id = ${meal_id}`)
}
